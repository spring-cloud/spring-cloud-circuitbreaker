...
public abstract class AbstractCircuitBreakerInterceptor implements BeanFactoryAware {
	...
	public Object invoke(final CircuitBreakerInvoker invoker, Object target, Method method) {
		...
		if (this.isReactiveMethod(method)) {
			...
			return invokeImperative(invoker, circuitBreakerName, target, fallbackMethod);
		}
	}
	...
	private Method findFallbackMethod(Object target, String fallbackMethodName) {
		...
		return ReflectionUtils.findMethod(target.getClass(), fallbackMethodName, Throwable.class);
	}
	...
	private Object invokeImperative(final CircuitBreakerInvoker invoker, String circuitBreakerName, Object target,
		...
		if (fallbackMethod == null) {
			...
			return cb.create(circuitBreakerName).run(invoker::invoke, t ->
				ReflectionUtils.invokeMethod(fallbackMethod, target, t)
			);
		}
	}
	...
	private Object invokeFlux(final CircuitBreakerInvoker invoker, String circuitBreakerName, Object target,
		Method fallbackMethod) {
		ReactiveCircuitBreakerFactory cb = this.beanFactory.getBean(ReactiveCircuitBreakerFactory.class);
		if (fallbackMethod == null) {
			return cb.create(circuitBreakerName).run((Flux<Object>) invoker.invoke());
		} else {
			return cb.create(circuitBreakerName).run((Flux<Object>) invoker.invoke(), (t) ->
				(Flux<Object>) ReflectionUtils.invokeMethod(fallbackMethod, target, t)
			);
		}
	}

	private Object invokeMono(final CircuitBreakerInvoker invoker, String circuitBreakerName, Object target,
		Method fallbackMethod) {
		ReactiveCircuitBreakerFactory cb = this.beanFactory.getBean(ReactiveCircuitBreakerFactory.class);
		if (fallbackMethod == null) {
			return cb.create(circuitBreakerName).run((Mono<Object>) invoker.invoke());
		} else {
			return cb.create(circuitBreakerName).run((Mono<Object>) invoker.invoke(), (t) ->
				(Mono<Object>) ReflectionUtils.invokeMethod(fallbackMethod, target, t)
			);
		}
	}
