...
public class Resilience4jBulkheadProvider {
	...
	public Resilience4jBulkheadProvider(
		...
		defaultConfiguration = id -> new Resilience4jBulkheadConfigurationBuilder()
				...
				.threadPoolBulkheadConfig(
						this.threadPoolBulkheadRegistry.getDefaultConfig())
				.build();
	}
	...
	public void configure(Consumer<Resilience4jBulkheadConfigurationBuilder> consumer,
			String... ids) {
		for (String id : ids) {
			Resilience4jBulkheadConfigurationBuilder builder = new Resilience4jBulkheadConfigurationBuilder();
			consumer.accept(builder);
			Resilience4jBulkheadConfigurationBuilder.BulkheadConfiguration configuration = builder
					.build();
	...
	public void addThreadPoolBulkheadCustomizer(Customizer<ThreadPoolBulkhead> customizer,
			String... ids) {
		for (String id : ids) {
			Resilience4jBulkheadConfigurationBuilder.BulkheadConfiguration configuration = configurations
					.computeIfAbsent(id, defaultConfiguration);
	...
	public <T> T run(String groupName, Supplier<T> toRun, Function<Throwable, T> fallback,
		...
		Supplier<CompletionStage<T>> bulkheadCall = decorateBulkhead(groupName, tags,
				toRun);
	...
	private <T> Supplier<CompletionStage<T>> decorateBulkhead(final String id,
			final io.vavr.collection.Map<String, String> tags,
		...
		if (bulkheadRegistry.find(id).isPresent()
			...
			Bulkhead bulkhead = bulkheadRegistry.bulkhead(id,
					configuration.getBulkheadConfig(), tags);
