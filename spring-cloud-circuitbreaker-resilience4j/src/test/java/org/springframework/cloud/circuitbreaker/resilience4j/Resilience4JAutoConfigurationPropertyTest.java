...
public class Resilience4JAutoConfigurationPropertyTest {
	...
	public void testThreadPoolBulkheadPropertiesPopulated() {
		ThreadPoolBulkheadRegistry threadPoolBulkheadRegistry = factory
				.getBulkheadProvider().getThreadPoolBulkheadRegistry();
		assertThat(threadPoolBulkheadRegistry).isNotNull();
		assertThat(threadPoolBulkheadRegistry.find("test_circuit")).isPresent();
		assertThat(threadPoolBulkheadRegistry.find("test_circuit").get()
				.getBulkheadConfig().getMaxThreadPoolSize()).isEqualTo(100);
	}

	@Test
	public void testBulkheadPropertiesPopulated() {
		BulkheadRegistry bulkheadRegistry = factory.getBulkheadProvider()
				.getBulkheadRegistry();
		assertThat(bulkheadRegistry).isNotNull();
		assertThat(bulkheadRegistry.find("test_circuit")).isPresent();
		assertThat(bulkheadRegistry.find("test_circuit").get().getBulkheadConfig()
	...
	public void testDefaultThreadPoolBulkheadPropertiesPopulated() {
		...
		assertThat(threadPoolBulkheadRegistry.find("default_circuitBreaker").get()
				.getBulkheadConfig().getMaxThreadPoolSize()).isEqualTo(50);
	}
